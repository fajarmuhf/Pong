...
public class PlayerControl : MonoBehaviour
{
    ...
    void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        else if (collision.gameObject.name.Contains("PowerUp"))
        {
            ...
            transform.localScale = new Vector3(1f,1.7f,1f);
            Invoke("NormalSize",5);
            GameObject.Find("GameManager").GetComponent<GameManager>().jumlahPowerUp--;
        }
        else if (collision.gameObject.name.Contains("FireBall"))
        {
            Destroy(collision.gameObject);
            if (name == "Player1") {
                GameObject.Find("GameManager").GetComponent<GameManager>().player1FireBall = true;
            }
            else
            {
                GameObject.Find("GameManager").GetComponent<GameManager>().player2FireBall = true;
            }
            GameObject.Find("GameManager").GetComponent<GameManager>().jumlahFireBall--;
        }
    }
